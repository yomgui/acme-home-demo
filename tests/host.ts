...
app.get("/", (_req, res) => {
  res
    .type("html")
    .send(
      '<!doctype html><html><head><title>Acme Home · MCP SDK preview</title><style>body{margin:0}iframe{display:block;width:100%;height:calc(100vh - 32px);border:0}header{height:32px;font:12px sans-serif;display:flex;gap:12px;align-items:center;padding:0 12px}#calls{margin-left:auto;max-width:50%;overflow:hidden;white-space:nowrap}</style></head><body><header>Acme Home · SDK preview <button id="fail">Fail next attention call</button><button id="teardown">Host teardown</button><span id="calls"></span></header><iframe title="MCP App" sandbox="allow-scripts"></iframe><script type="module" src="/host.js"></script></body></html>',
    );
});
